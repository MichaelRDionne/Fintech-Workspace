...
def  generate_account(w3): 
    ...
    private, account = wallet.derive_account('eth')
    account = w3.eth.account.privateKeyToAccount(private)
    return account


def get_balance(w3, address):
    """Display the balance of an Ethereum account."""
    wei_balance = w3.eth.getBalance(address)
    ether = w3.fromWei(wei_balance, "ether")
    return ether

def send_transaction(w3, account, receiver, ether):
    """Send an Ethereum transaction to a Fintech Finder candidate."""
    # medium gas price strategy
    w3.eth.setGasPriceStrategy(w3.eth.gasPriceStrategy.medium_gas_price_strategy)
    ...
    gas_estimate = w3.eth.estimateGas(
        {"from": account.address, "to": receiver, "value": wei_value })
